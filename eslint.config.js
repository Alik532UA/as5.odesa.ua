...
export default ts.config(
	...
	...svelte.configs['flat/recommended'],
	{
		...
		rules: {
			...
			// --- Глушник, який більше нічого не глушить.
			//
			// Правило є у пресеті; тут воно підняте явно, бо вже спрацювало не
			// формально. `svelte-ignore` у Svelte діє не лише на наступний
			// елемент, а й на все його ПІДДЕРЕВО, і в `PianoModal` два рядки,
			// написані про тло модалки, мовчки накривали сімнадцять клавіш —
			// `<div onclick>` без ролі й таб-порядку. Коли клавіші стали
			// `<button>`, саме це правило одразу сказало, що другий глушник
			// зайвий. Без нього він лишився б у файлі й накрив би наступну таку
			// помилку так само тихо.
			...
			// --- CODE-QUALITY-v8, анти-патерни: `console.log` у продакшн-коді.
			// Було десять викликів, і половина з них — у гарячих місцях: на
			// кожній зміні тла, на кожному ресайзі, на кожному монтуванні. Вони
			// їхали у бандл відвідувача разом із `new Date().toISOString()`
			// поруч, тобто виконувалися й тоді, коли консоль ніхто не відкривав.
			//
			// `warn` і `error` лишаються дозволеними: це не сліди налагодження,
			// а свідомі повідомлення фасаду сховища, міграції та `errorLogger` —
			// єдиних місць, де в цьому проєкті взагалі є що сказати консолі.
			// Окремого LogService тут немає, тому межа проходить саме тут.
			'no-console': ['error', { allow: ['warn', 'error'] }],

			// --- Розібраний борг: правила, доведені до нуля й підняті в 'error' ---
			// Комент вище цього блоку колись обіцяв, що кожне з них має стати
			// 'error'. Стало: 2026-08-19 їхні числа дійшли до нуля, і 'error'
			// тепер тримає цей нуль перевіреним (CODE-QUALITY-v8 § 6.4.1,
			// останній пункт). Повернути 'warn' означає знову дозволити борг —
			// це видно в diff, на відміну від його тихого накопичення.

			// SVELTE-UI-v8 § 1.5, HIGH. Було 24 місця, потім 20, тепер 0. Ціна
			// ключа не нульова: дублікат кидає помилку в РАНТАЙМІ, а не на
			// збірці, тож ключ береться з поля, яке код і так вважає унікальним
			// (id, slug, src), а не з першого-ліпшого рядка. Там, де елементи
			// навмисно повторюються — клони каруселі, подвоєний список для
			// стрічки — ключем служить позиція, і це записано поруч.
			'svelte/require-each-key': 'error',

			// CODE-QUALITY-v8 § 1, HIGH. Було 4 місця, потім 2, тепер 0.
			// Останній `as any` приховував не відсутність типу, а його
			// наявність: `color` уже був у `NewsItemSchema`.
			'@typescript-eslint/no-explicit-any': 'error',

			// --- Борг, що лишається: 5 місць ---

			// SEO-v8 § 1.5. Було 13, потім 11, тепер 5. Різниця з `${base}/…` не
			// косметична: resolve() типізований проти списку реальних маршрутів.
			// П'ять, що лишилися, — там, де адреса приходить ВИРАЗОМ, який
			// лінтер перевірити не може: два зовнішні посилання соцмереж і одне
			// конкурсу зі словника, `replaceState` та посилання на неіснуючі
			// `/news/*` у чернетці. Тобто це не борг коду, а межа правила —
			// але 'warn' із числом чесніше за 'off' із поясненням.
			'svelte/no-navigation-without-resolve': 'warn',

			// Було 16 місць, потім 11, тепер 0. Службові імена не рахуються
			// боргом: `_`-префікс, залишок деструктуризації та параметр catch —
			// це ідіоми, а не техборг.
	...
	{
		// Скрипти в `scripts/` — CLI-інструменти, і `console.log` тут не слід
		// налагодження, а ЄДИНИЙ спосіб віддати результат: саме його читає
		// людина в терміналі й крок CI у логу прогону. Заборона стосується
		// коду, який їде в браузер відвідувача, а не цих файлів.
		files: ['scripts/**'],
		rules: {
			'no-console': 'off'
		}
