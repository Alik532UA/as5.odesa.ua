 ...
 *
 * ## Межа цього файлу — і чому поруч стоїть окремий процес
 *
 * Усе тут — СТАТИЧНИЙ розбір: «файл імпортує vitest, vitest у залежностях є,
 * конфіг у корені є». Про маску `include` цей файл не знає нічого, і знати не
 * може: він сам лежить під нею. Звідси дві діри, які він не закриває
 * структурно, а не через недогляд:
 *
 *   1. `tests/foo.test.ts` пройде його повністю й не запуститься ніде —
 *      `include` vitest тут `src/**`, а Playwright такого файлу не бачить.
 *   2. Звужена маска `include` вимикає разом із тестами й цю перевірку.
 *
 * Обидві закриває `scripts/check-test-discovery.mjs`
 * (`npm run check:test-discovery`, AI-AGENT-PITFALLS-v9 § 1.3.1,
 * `PIT-TEST-DISCOVERY-PROCESS`): він ПИТАЄ раннера, що той зібрав, і живе
 * окремим процесом, тому маска його не дістає. Цей файл лишається як швидка
 * половина з кращою діагностикою — і як єдиний, хто звіряє документи для
 * агента з `package.json`.
 *
