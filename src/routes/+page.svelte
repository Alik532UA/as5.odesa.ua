...
<style>
	...
	.gallery-bento__subtitle {
		font-size: 1.2rem;
		/*
		 * Приглушення КОЛЬОРОМ, а не `opacity: 0.7`.
		 *
		 * Прозорість змішує текст із тлом, і саме змішаний колір бачить читач:
		 * заміряно `#748a97` на `#f5fafd` — 3.42:1 при потрібних 4.5 (axe,
		 * 2026-08-23). У джерелах цього не видно взагалі: `--color-body-text` сам
		 * дає 7.4:1, тобто виглядає бездоганно, а порушення народжується вже в
		 * браузері. Це один із двох класів, які `src/contrast.test.ts` називає
		 * непокритими за побудовою.
		 */
