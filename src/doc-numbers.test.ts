...
/**
 * Обидва документи для агента називають ОДНУ версію пакета
 * (AI-AGENT-PITFALLS-v9 § 5.5.2, `PIT-DOC-FACTS`: не лише числа — шляхи теж).
 *
 * ## Чому це не косметика
 *
 * До 2026-09-10 і `AGENTS.md`, і `PROJECT-CONTEXT.md` називали джерелом
 * стандартів `selection_criteria/v8`. Тека `v8` при цьому нікуди не зникла —
 * саме тому помилка була тихою в найгіршому сенсі: посилання виглядало
 * робочим, агент читав ІСНУЮЧИЙ пакет і не бачив жодного правила, доданого у
 * v9. Аудит проти 9.2 знайшов десять таких правил без гейта, і кожне з них
 * було невидиме рівно через цей рядок.
 *
 * ## Межа перевірки, і вона названа чесно
 *
 * Пакет живе ПОЗА репозиторієм (`../sveltekit-canon/`), і в CI його немає.
 * Тому довести «версія чинна» цей гейт не може й не вдає, що може. Він
 * доводить те, що доводиться з наявних файлів: обидва документи називають одну
 * версію й один шлях. Клас, який це ловить, у проєкті вже стріляв —
 * `AGENTS.md` три дні поспіль суперечив `PROJECT-CONTEXT.md` щодо Playwright,
 * і не червоніло нічого (`src/test-runners.test.ts`).
 *
 * Зворотний експеримент (§ 1.1): повернути в `AGENTS.md` шлях `.../v8` —
 * перевірка червоніє на розходженні. Прогнано.
 */
describe('версія пакета названа однаково в обох документах (§ 5.5.2)', () => {
	/** `selection_criteria/v9`, `selection_criteria/**v9**` — жирне теж. */
	const PACKAGE_PATH = /selection_criteria\/\*{0,2}(v\d+)\*{0,2}/g;

	const versionsIn = (source: string) => [...new Set([...source.matchAll(PACKAGE_PATH)].map((m) => m[1]))];

	it('перевірка жива: шлях до пакета згадано в обох документах', () => {
		expect(versionsIn(AGENTS), 'AGENTS.md не називає пакет узагалі').not.toEqual([]);
		expect(versionsIn(CONTEXT), 'PROJECT-CONTEXT.md не називає пакет узагалі').not.toEqual([]);
	});

	it('усередині кожного документа версія одна', () => {
		expect(versionsIn(AGENTS), 'AGENTS.md називає дві різні версії пакета').toHaveLength(1);
		expect(versionsIn(CONTEXT), 'PROJECT-CONTEXT.md називає дві різні версії пакета').toHaveLength(
			1
		);
	});

	it('обидва документи називають ту саму версію', () => {
		expect(
			versionsIn(AGENTS)[0],
			'AGENTS.md і PROJECT-CONTEXT.md ведуть агента в різні пакети — ' +
				'він прочитає той, що трапиться першим, і не побачить правил другого'
		).toBe(versionsIn(CONTEXT)[0]);
	});
});
