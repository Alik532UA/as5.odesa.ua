...
/**
 * Чинні перевищення § 7 із числами на 2026-08-20. Список **тільки
 ...
 *
 * Число — це `split('
').length`, тобто на одиницю більше за `wc -l`: у файлі
 * з кінцевим переносом рядків рівно стільки. Записувати сюди результат `wc -l`
 * означало б зробити межу на одиницю жорсткішою, ніж вона є, і дізнатися про це
 * лише в момент, коли хтось не зможе дописати рядок.
 */
const SIZE_DEBT: Record<string, number> = {
	...
	// 355 → 335: контакт підвалу став `FooterContactItem.svelte` разом зі своїми
	// стилями. Розділення завів саме цей інваріант — правило WCAG про розмір
	// цілі не вміщалося у файл за межею, і замість того, щоб підняти число,
	// довелося прибрати з підвалу три копії тієї самої розмітки.
	'src/lib/components/FooterSection.svelte': 335
};
