...
describe('кожна модалка бере focusTrap (ACCESSIBILITY-v8 § 6)', () => {
	...
	it('модалки в проєкті знайдено — перевірка жива', () => {
		...
		//
		// 2 → 3 (2026-09-13): додався `PhotoLightbox` — світлина на весь екран
		// на головній і в «Про школу». Число, а не `toBeGreaterThan`: воно й
		// ловить модалку, додану повз `focusTrap`, бо тоді наступна перевірка
		// назве її поіменно, а ця скаже, що їх стало більше.
		expect(dialogs.length, 'жодного aria-modal у розмітці — перевірка сліпа').toBe(3);
	});
