<script lang="ts">
	...
	let fishY = $derived.by(() => {
		...
		return baseline + amplitude * Math.sin(normalizedX * Math.PI * 2 * frequency + phase);
	});
...
<div class="wave-container {className}">
	...
	>
		...
		{#if showFish && isFishActive}
			<g transform="translate({fishX}, {fishY - 15}) scale(0.08)">
