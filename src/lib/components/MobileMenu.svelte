...
<style>
	...
	/*
	 * Стеля висоти в центрованому оверлеї (FLUID-SIZING-v8, анти-патерн
	 * CRITICAL). Сьогодні меню НЕ обрізається, і це заміряно, а не припущено:
	 * на 667×375 — найкоротшому вікні, де бургер узагалі показується, — nav має
	 * 363 px при 375 px екрана, тобто по 6 px запасу зверху й знизу.
	 *
	 * Шести пікселів вистачає рівно доти, доки пунктів пʼять. Шостий коштує
	 * 61 px (`--space-xl` + рядок), і меню поїде за екран — а батько центрує й
	 * ріже з ОБОХ боків, тож ні першого пункту, ні кнопки «Вступ» не буде
	 * видно, і доскролити до них не можна взагалі. Симптом при цьому виглядає
	 * як «меню трохи менше», а не як поломка.
	 */
	nav {
		max-height: 100dvh;
		overflow-y: auto;
