<script lang="ts">
	...
	/**
	 * Ідентифікатор обтинання — з `$props.id()`, а не з `Math.random()`
	 * (ACCESSIBILITY-v8 § 4.2.1).
	 *
	 * Потрібен саме УНІКАЛЬНИЙ: на головній цей компонент стоїть двічі (герой і
	 * підвал), на `/test` — тричі, а `url(#…)` бере ПЕРШИЙ збіг у документі.
	 * Спільний рядок замість цього означав би, що дві хвилі обтинаються однією
	 * маскою.
	 *
	 * Чому не випадковість. `Math.random()` дає різне значення на сервері й у
	 * браузері, тож звʼязок «елемент ↔ посилання на нього» лишається цілим лише
	 * доти, доки обидва кінці малює один бік. Тут це поки що так: `<clipPath>`
	 * стоїть під `{#if showFish && isFishActive}`, а `isFishActive` на старті
	 * `false` — у `build/*.html` цієї розмітки немає жодного байта, і симптому
	 * теж немає (перевірено grep-ом по `build/`). Тобто це правило без
	 * сьогоднішнього наслідку — і рівно тому воно й доживає до дня, коли
	 * наслідок зʼявиться: досить показати рибку одразу.
	 *
	 * `$props.id()` (Svelte 5.20+) дає значення, стабільне між сервером і
	 * клієнтом і різне для кожного екземпляра — обидві вимоги одразу.
	 *
	 * Два рядки, а не один: `$props.id()` дозволено лише як ІНІЦІАЛІЗАТОР
	 * оголошення на верхньому рівні компонента. Усередині шаблонного рядка
	 * компілятор кидає `props_id_invalid_placement`, і це помилка, а не
	 * попередження — `npm run check` її ловить.
	 */
	const uid = $props.id();
