 ...
 */
/**
 * `vh` замість `dvh` (FLUID-SIZING-v8, анти-патерн MEDIUM).
 *
 * `100vh` — це LARGE viewport: висота вікна, яка буває лише зі схованою панеллю
 * браузера. Поки панель видно, блок вищий за екран рівно на її висоту. Дефект
 * тихий: на десктопі різниці немає взагалі, а на телефоні виглядає як «сторінка
 * чомусь прокручується», а не як помилка розміру.
 *
 * Лапка перед числом виключає `dvh`/`svh`/`lvh` без окремого фільтра: у них
 * перед `vh` стоїть літера, а не цифра.
 */
const VIEWPORT_HEIGHT = /(?<![\w.-])\d+(?:\.\d+)?vh(?![\w-])/g;

/**
 * Свідомі `vh`, кожен із причиною.
 *
 * Єдиний — ширина повернутого на 90° піаніно в портретній орієнтації: там `vh`
 * задає саме ШИРИНУ, тобто вимірює поперечну вісь екрана, і `dvh` зробив би
 * ширину клавіатури залежною від того, чи видно панель браузера — тобто
 * повертав би розкладку під час прокрутки.
 */
const VH_ALLOWED: readonly string[] = ['src/lib/components/ui/PianoModal.svelte: 90vh'];
...
describe('гнучкі розміри', () => {
	...
	it('висота міряється в dvh, а не vh', () => {
		const bad: string[] = [];
		for (const f of files) {
			const source = readFileSync(join(ROOT, f), 'utf8')
				.replace(/\/\*[\s\S]*?\*\//g, '')
				.replace(/<!--[\s\S]*?-->/g, '');
			for (const m of source.matchAll(VIEWPORT_HEIGHT)) {
				const entry = `${f}: ${m[0].trim()}`;
				if (VH_ALLOWED.some((allowed) => entry.startsWith(allowed))) continue;
				bad.push(entry);
			}
		}
		expect(
