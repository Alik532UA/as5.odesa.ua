 ...
 * здатна валідувати суфікси.
 *
 * ЩО ЦЯ ПЕРЕВІРКА НЕ ЛОВИТЬ, і це не оцінка, а замір. Дублікат — властивість
 * СТОРІНКИ, а не файлу: перевірка нижче бачить лише повтор у межах одного
 * компонента. Два компоненти з чистими власними списками дають зіткнення,
 * щойно опиняться в одному DOM, і тут це лишається зеленим. Заміряно
 * зворотним експериментом: `mobile-nav-{item.key}-link` у `MobileMenu.svelte`
 * замінено на вже зайнятий шапкою `nav-{item.key}-link` — тут 8 із 8
 * пройдено, а `tests/testid-runtime.spec.ts` червоніє на відкритому меню.
 *
 * Рантайм-дублікати ловить `tests/testid-runtime.spec.ts`
 * (`GATE-TESTID-RUNTIME`). До 2026-08-27 цей рядок посилався на нього як на
 * наявний, а файлу не існувало — гейт був обіцянкою, і саме тому вона тут
 * тепер названа файлом, а не раннером.
 */
