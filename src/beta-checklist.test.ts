...
describe('чеклист бета-тестування', () => {
	...
	it('у кожній вкладці є пункт для людини і пункт-межа (§ 5.4)', () => {
		...
		for (const tab of BETA_TABS) {
			 ...
			 *
			 * Контрольна група корисна доти, доки лишається групою, а не списком:
			 * кожен `covered` витрачає час живої людини там, де автотест уже
			 * дивиться. У сусідньому проєкті набору перекіс дійшов до 26 `covered`
			 * проти 15 `manual`, тобто половина чеклиста була контрольною групою.
			 */
			const n = (level: string) => own.filter((c) => c.coverage === level).length;
			if (n('covered') > n('manual')) {
				bad.push(`${tab.id}: covered ${n('covered')} > manual ${n('manual')} — контрольна`);
			}
		}
		expect(bad, bad.join('\n')).toEqual([]);
	});
