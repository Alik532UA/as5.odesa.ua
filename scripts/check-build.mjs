...
function readConst(file, re, what) {
  ...
  if (!m) {
    console.error(
      `check-build: не знайдено ${what} у ${file}. Гейт зупинено — без цього він перевіряв би не те.`,
    );
    process.exit(1);
  }
  return m[1];
...
function readList(file, re, what) {
  ...
  if (items.length === 0) {
    console.error(
      `check-build: ${what} у ${file} порожній — перевірка службових сторінок була б мертвою.`,
    );
    process.exit(1);
  }
  return items;
...
if (files.length < 6) {
  console.error(
    `Знайдено лише ${files.length} HTML — перевірка мертва, очікується 7+.`,
  );
  process.exit(1);
}
...
function checkInlineScripts(file, html) {
  ...
  for (const match of html.matchAll(INLINE)) {
    ...
    if (match.index < cspAt) {
      fail(
        `${file}: інлайн-скрипт стоїть ВИЩЕ мета-політики — вона його не покриває`,
      );
      continue;
    }
    ...
    if (!policy.includes(hash)) {
      fail(
        `${file}: інлайн-скрипт без хеша в script-src — браузер його заблокує (${hash})`,
      );
    }
  }
...
for (const file of files) {
  ...
  if (!isShell) {
    ...
    if (text.length < MIN_BODY_TEXT) {
      fail(
        `${file}: видимого тексту ${text.length} символів (мінімум ${MIN_BODY_TEXT})`,
      );
    }
  }
  ...
  if (!isShell) {
    ...
    if (title.length < 5)
      fail(`${file}: title відсутній або надто короткий — «${title}»`);
    ...
    if (/^[a-z][\w.]*\.[\w.]+$/.test(title)) {
      fail(
        `${file}: у title потрапив КЛЮЧ перекладу, а не переклад — «${title}»`,
      );
    }
    ...
    if (robotsTags.length !== 1) {
      ...
      // поруч — `noindex`.
      fail(
        `${file}: <meta name="robots"> знайдено ${robotsTags.length} разів, очікується 1`,
      );
    }
    const robots =
    ...
    if (shouldHide) {
      if (canonicals.length !== 0) {
        fail(
          `${file}: службова сторінка оголошує canonical (${canonicals.length}) — разом із noindex це суперечливі сигнали`,
        );
      }
    } else if (canonicals.length !== 1) {
      fail(
        `${file}: canonical знайдено ${canonicals.length} разів, очікується 1`,
      );
    } else {
...
for (const page of REQUIRED_PAGES) {
  if (!existsSync(join(BUILD, page)))
    fail(`немає build/${page} — сторінка не згенерована`);
}
...
for (const route of HIDDEN_ROUTES) {
  ...
  if (!existsSync(join(BUILD, page))) {
    fail(
      `немає build/${page} — службова сторінка не згенерована (перевір prerender.entries)`,
    );
  }
...
for (const [page, markers] of Object.entries(REQUIRED_CONTENT)) {
  ...
  for (const marker of markers) {
    if (!html.includes(marker)) {
      fail(
        `build/${page}: у prerendered HTML немає «${marker}» — блок сховано за клієнтським станом`,
      );
    }
  }
}
    ...
    if (!existsSync(local)) {
      ...
      for (const loc of locs) {
        if (!loc.startsWith(`${SITE_ROOT}/`))
          fail(`sitemap: адреса не з цього сайту — ${loc}`);
        ...
        if (HIDDEN_PAGES.has(asFile)) {
          fail(
            `sitemap оголошує ${loc}, а сторінка позначена noindex — списки суперечать`,
          );
        }
      }
      ...
      for (const file of files) {
        ...
        const expected =
          name === "index.html"
            ? `${SITE_ROOT}/`
            : `${SITE_ROOT}/${name.replace(/\.html$/, "")}`;
        if (!inSitemap.has(expected)) {
          fail(
            `${name}: сторінка індексується, а в sitemap її немає — очікувався ${expected}`,
          );
        }
      }
    }
...
for (const file of files) {
  ...
  const refs = [
    ...new Set(
      [...html.matchAll(/["'(]\.?\/?(_app\/immutable\/[^"')]+\.js)/g)].map(
        (m) => m[1],
      ),
    ),
  ];
  ...
  if (refs.length === 0) {
    fail(
      `${name}: жодного посилання на _app/immutable/*.js — структура build/ змінилася, бюджет міряти нічим`,
    );
    continue;
  }
  ...
  if (kb > JS_BUDGET_KB) {
    fail(
      `${name}: ${kb} КБ JS gzip — бюджет ${JS_BUDGET_KB} КБ (PERFORMANCE-v8 § 1)`,
    );
  }

  /*
   * КАНАРКА СТИЛІВ (OBSERVABILITY-v9 § 2.2.1, `OBS-LHCI-REAL-PAGES`).
   *
   * Половина вище доводить, що сторінка тягне JS, і робить це заради бюджету.
   * Друга половина — CSS — доти не перевірялася нічим, а вона й є та, через яку
   * Lighthouse бреше найголосніше: сторінка БЕЗ стилів має чудові бали
   * продуктивності, бо нічого не малює й нічого не вантажить. У сусідньому
   * `adoptananimal` сім сторінок мірялися саме так — сервер піднімався без
   * базового шляху, стилі віддавали 404, і звіт був зелений.
   *
   * Тут це перевіряється РАЗОМ із JS і в тому самому місці: якщо канарка
   * стоїть у конфізі Lighthouse, вона мовчить рівно тоді, коли Lighthouse не
   * запустився. Стоячи над `build/`, вона каже те саме про кожну сторінку — і
   * до того, як звіт узагалі почнеться.
   */
  const cssRefs = [
    ...new Set(
  ...
  if (cssRefs.length === 0) {
    fail(
      `${name}: жодного <link rel="stylesheet"> — сторінка без стилів, і Lighthouse дасть їй чудові бали саме за це`,
    );
  }
  for (const ref of cssRefs) {
    if (!existsSync(join(BUILD, ref)))
      fail(`${name}: посилається на стиль ${ref}, а файлу в build/ немає`);
  }
}
...
// ---------------------------------------------------------------------------
// Сторінки, які МІРЯЄ Lighthouse, придатні для заміру
// (OBSERVABILITY-v9 § 2.2.1, `OBS-LHCI-REAL-PAGES`).
//
// Умова одна й вона вирішує, чи стане крок `Lighthouse Audit` червоним від
// народження: категорія SEO штрафує за `noindex`, і сторінка з ним дає 66 при
// порозі 90 на рівні `error` (заміряно 2026-09-10 на `/test` і
// `/beta-test-checklists`). Генератор адрес виключає службові за
// `HIDDEN_ROUTES` — але то ДЖЕРЕЛА; чи справді на сторінці немає `noindex`,
// видно лише в зібраному HTML, бо мета-тег ставить макет за `isHiddenRoute`.
//
// Тут же перевіряється й існування файлу: адреса, якої немає в `build/`, дає
// Lighthouse 404 і бали, що не означають нічого.
const { lighthouseUrls } = await import("./lhci-urls.cjs");
for (const url of lighthouseUrls()) {
  const name = url.slice(url.lastIndexOf("/") + 1);
  const path = join(BUILD, name);
  if (!existsSync(path)) {
    fail(`${name}: Lighthouse має міряти цю сторінку, а файлу в build/ немає`);
    continue;
  }
  const html = readFileSync(path, "utf8");
  if (/<meta[^>]+name="robots"[^>]+content="[^"]*noindex/i.test(html)) {
    fail(
      `${name}: сторінка несе noindex і водночас стоїть у переліку Lighthouse — ` +
        "категорія SEO дасть їй ~66 при порозі 90, тобто гейт стане червоним за " +
        "те, що зроблено навмисно. Додати маршрут у HIDDEN_ROUTES (src/lib/config/site.ts)",
    );
  }
}
...
console.log(
  `Збірка перевірена: ${files.length} HTML, sitemap на місці, проблем немає.`,
);
...
console.log(
  `JS gzip на сторінку (бюджет ${JS_BUDGET_KB} КБ): ${jsSizes.join(", ")}`,
);
