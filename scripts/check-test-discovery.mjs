 ...
 *
 * Числа заміряні цим самим скриптом і записані з запасом на видалення одного
 * файлу: перевірка стоїть проти обвалу покриття, а не проти рефакторингу.
 * Скрипт друкує обидва числа щоразу, тож підняти межу після додавання тестів
 * можна не вгадуючи.
 *
 * Запас — розмір найбільшого файлу перевірок (2026-09-11: `src/ci.test.ts`, 24
 * тести), а не частка від суми: частка зростає разом із покриттям, тобто межа
 * тим слабша, чим більше тестів. Ратчет так не працює.
 *
 * `vitest`: 250 → 297. Заміряно 2026-09-11 — 321 тест у 39 файлах, після того
 * як аудит проти v9.2 додав `seo-head-owner`, `a11y-static` і
 * `lib/services/resetService`. Стара межа 250 після цього означала б, що можна
 * мовчки втратити 71 тест — тобто три файли, — і гейт лишиться зеленим.
 *
 * `playwright`: 60 лишається. Кількість E2E не змінилася (111 у 7 файлах), а
 * піднімати межу, нічого не додавши, означало б звузити її навмання.
 */
const MIN_TESTS = { vitest: 297, playwright: 60 };
